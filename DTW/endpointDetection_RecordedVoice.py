...
for i in range(10) :
    for j in range(6) :
        ...
        m = 0
        while m < len(N) -1:
            save_wave_file("./RecordedVoice-EndPointed/" + str(i + 1) + "-" + str(j + 1) + ".wav", wave_data[N[m] * 256 : (N[m+1]+5) * 256])
            m = m + 2
        


# 利用 HCopy 工具对录取的语音进行 MFCC 特征提取
os.chdir("C:\\Users\\30753\\Desktop\\Smart_home\\DTW\HTK-EndPointedVoice")
os.system("hcopy -A -D -T 1 -C tr_wav.cfg -S .\list.scp")
os.chdir("C:\\Users\\30753\\Desktop\\Smart_home\\DTW")